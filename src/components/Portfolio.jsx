...
const articles = [
  ...
  function ArticleSection({ title, subtitle, description, image }) {
    ...
    return (
      <article className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-10 mt-12 mb-[6rem] rounded-lg p-6 md:p-12 border shadow-2xl bg-gradient-to-br from-blue-200 via-indigo-100 to-slate-300 mx-4 md:mx-8  hover:scale-105 transition transform duration-700">
        <img src={image} alt="" className="w-full md:w-[40rem] rounded-lg" />
        <div className="mr-0 md:mr-16 space-y-4 flex flex-col justify-center">
          <h3 className="hidden md:inline-block md:text-4xl font-black text-transparent bg-indigo-400 bg-clip-text rounded-md text-center md:mb-[6rem] hover:scale-105">{title}</h3>
          ...
          className={`md:max-h-full ${isTextHidden ? 'max-h-0' : 'max-h-full'} overflow-hidden transition-max-height duration-300 ease-in-out text-center text-[2ch]`}
        >
          <p>{description}</p>
      </div>
